<template>
	<view class="common-list u-f-jsb animated fadeInLeft fast">
		...
		<view class="common-list-r">
			...
			<view class="u-f-ajc" style="flex-direction: column;">
				...
				<block v-for="(pic,index) in item.morepic" :key="index">
					<!-- 有图就显示 -->
					<image  :src="pic" style="margin-bottom:20upx;" mode="widthFix" lazy-load @tap="imgdetail(index)"></image>
				</block>
		        <!-- 视频 -->
			...
			<view class="u-f-ac u-f-jsb">
				<view>{{item.path}}</view>
				<view class="u-f-ac ">
					<view class="icon oconfont zhuanfa">
						{{item.sharenum}}
				    </view>
